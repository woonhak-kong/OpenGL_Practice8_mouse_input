...
{
	std::cout << "Clicked: " << (btn == 0 ? "left " : "middle/right ") << (state == 0 ? "down " : "up ") <<
		"at " << x << "," << y << std::endl;

	if (btn == GLUT_LEFT_BUTTON && state == GLUT_DOWN) std::cout << "Left button down" << std::endl;
	if (btn == GLUT_MIDDLE_BUTTON && state == GLUT_DOWN) std::cout << "Middle button down" << std::endl;
	if (btn == GLUT_RIGHT_BUTTON && state == GLUT_DOWN) std::cout << "Right button down" << std::endl;
}
